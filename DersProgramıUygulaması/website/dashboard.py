...
from .models import User, Department, LessonType, SyllabusOptions, Syllabus, Lesson, Classroom
...
def syllabus_create():
    sum_hpw = SyllabusOptions.select(fn.SUM(SyllabusOptions.hpw)).scalar()
    count_of_clasrooms = Classroom.select().count()

    if count_of_clasrooms*40 < sum_hpw:
        flash("Yeterli Derslik yok!", "danger")
        return redirect(url_for('dashboard.index'))
...
def syllabus_delete(id):
    ...
    try:
        excel_folder_location = (current_app.config.get('ROOT_URL') / 'excels' / 'output' / syllabus.excel_folder)

        shutil.rmtree(excel_folder_location.resolve())
...
# ! Derslikler Başlangıç

# Derslik Anasayfa
@dashboard.route('/classroom')
def classroom_page():
...
# Derslik Ekleme
@dashboard.route('/classroom/add', methods=['GET', 'POST'])
def classroom_add():
    if request.method == 'POST':
        name = request.form.get('name')
        code = request.form.get('code').lstrip().upper()
        ...
        flash(f"Derslik başarıyla kaydedildi!", "success")
        return redirect(url_for('dashboard.classroom_page'))
    else:
        return render_template("classroom/add.html")

# Derslik Silme
@dashboard.route('/classroom/delete/<int:id>')
def classroom_delete(id):
    classroom = Classroom.select().where(Classroom.id == id).get_or_none()

    if not classroom:
        flash("Derslik bulunamadı!", "danger")
        return redirect(url_for('dashboard.classroom_page'))

    Lesson.delete().where(Lesson.classroom == classroom).execute()

    classroom.delete_instance(recursive=False)

    flash("Başarıyla silindi!", "info")
    return redirect(url_for('dashboard.classroom_page'))
