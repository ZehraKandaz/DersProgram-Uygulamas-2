...
from .models import Department, User, SyllabusOptions, Lesson, Syllabus, LessonType, Excel, Classroom
...
def get_available_classrooms(syllabus:Syllabus, day:int, hour:int) -> list:
    all_classrooms = [classroom for classroom in Classroom.select()]

    for classroom in all_classrooms:
        if Lesson.select().where(
            Lesson.syllabus == syllabus,
            Lesson.day==day,
            Lesson.start_hour==hour,
            Lesson.classroom==classroom
        ).get_or_none():
            all_classrooms.remove(classroom)
...
def place_lesson(syllabus_options:SyllabusOptions, syllabus:Syllabus, shared=False):
    ...
    while True:
        ...
        # Eğer yüz yüze ise  yeterli Derslik var mı kontrol et
        if not syllabus_options.lesson_type.online:
            if available_classrooms := get_available_classrooms(syllabus, day, hours[hour_index]):
                classroom = None
                if hour_index != 0:
                    last_lesson = Lesson.select().where(
                        Lesson.start_hour == hours[hour_index-1],
                        Lesson.day == day,
                        Lesson.options == syllabus_options,
                        Lesson.syllabus == syllabus,
                    ).get_or_none() # Bu ayarları kullanan aynı gün içerisindeki son dersi seç

                    if last_lesson and last_lesson.classroom in available_classrooms: # Eğer bu dersin dersliği kullanılabilir durumda ise dersliği seç
                        classroom = last_lesson.classroom

                if not classroom:
                    classroom = choice(available_classrooms)

            else: # Yeterli derslik yoksa bir sonraki gün veya saate geç
                hour_index += 1
                if hour_index >= len(hours):
                    hour_index = 0
                    day += 1
                    if day == 6: # Sonraki gün yoksa hata ver
                        return (False, f"Yeterli Derslik Yok Lütfen Derslikleri Güncelleyin")

                continue # Sonraki gün varsa devam edip dersliği tekrar kontrol et
        else:
            classroom = None
...
def get_lessons(syllabus, department, grade):
    table = [
        [None for _ in range(10)] for _ in range(5)
    ]
