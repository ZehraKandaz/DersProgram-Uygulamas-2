...
class Lesson(BaseModel):
    ...
    classroom = ForeignKeyField(Classroom, null=True, backref='classrooms') # Derslikler -> Null olabilir çünkü online dersler için sınıf gerekmez

    def __str__(self):
        rslt = f"""{self.type.name} - {self.teacher.name} - """

        if self.type.online:
            rslt += f"Online"
        else:
            rslt += f"Yüzyüze - {self.classroom.code}"

        return rslt
    class Meta:
        table_name = 'lessons'

db.create_tables([Department, User, Lesson, SyllabusOptions, LessonType, Syllabus, Excel, Classroom],safe=True)
